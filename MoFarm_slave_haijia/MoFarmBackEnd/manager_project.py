...
from .task_slave import TaskInSlave
...
class ProjectManager(object):
    ...
    # modified by zhj, 2022-5-4
    def run_project_slave(self, task_id, project_id, project_json):
        '''
        功能：训练节点启动任务
        '''

        # 添加任务,启动任务
        task = TaskInSlave(task_id, project_id, project_json)
        threading.Thread(target=task.start).start()
        print(time_format(), '任务', task_id, '启动.')
        ...
        # if project_id%2 == 0:
        #     cmd_line = 'python /home/jmlv/MoFarm_slave_haijia/MoFarmBackEnd/src/medmnist.py'
        #     t = threading.Thread(target=run_cmd, args=(cmd_line,))
        #     task['status'] = 'START'
        #     task['start_time'] = time.time()
        #     t.isDaemon = True
        #     # loacl_slave.add_task(task, t)
        #     t.start()
        #     print('任务 ', project_id, ' 已启动训练进程')
        # else:
        #     cmd_line = 'python /home/jmlv/MoFarm_slave_haijia/MoFarmBackEnd/src/fashion_mnist.py'
        #     t = threading.Thread(target=run_cmd, args=(cmd_line,))
        #     task['status'] = 'START'
        #     task['start_time'] = time.time()
        #     t.isDaemon = True
