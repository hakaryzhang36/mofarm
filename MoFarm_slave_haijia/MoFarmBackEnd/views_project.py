...
def run_project(request):
    ...
    manager = ProjectManager()
    success = manager.run_project(id)
...
def run_project_slave(request):
    '''
    训练节点启动任务接口
    '''
    response = {}
    data_json = json.loads(request.body)
    print(time_format(), '收到data json：\n', data_json)
    
    task_id = data_json['task_id']
    project_id = data_json['project_id']
    project_json = data_json['project_json']
    
    manager = ProjectManager()
    success = manager.run_project_slave(task_id=task_id, project_id=project_id, project_json=project_json)
